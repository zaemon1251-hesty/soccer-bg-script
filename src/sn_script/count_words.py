...
class Config(object):
    ...
    results_csv = datetime.datetime.utcnow().astimezone(tzinfo).strftime("%Y%m%d%H%m") + 'word_counts.csv'


def main(args: Config) -> int:
    args = Config()
    dataset_Test  = dataset.SoccerNetCaptions(path=args.SoccerNet_path, features=args.features, split=args.split_test, version=args.version, framerate=args.framerate, window_size=args.window_size_caption)
...
def process_texts(texts: list[str], processor: dataset.SoccerNetTextProcessor) -> list[list[int]]:
    results = []
    for text in texts:
